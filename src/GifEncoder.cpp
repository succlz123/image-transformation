...
std::vector<uint8_t> GifEncoder::addImage(const std::vector<uint32_t> &original, uint32_t delay,
    ...
    if (qType == QuantizerType::Octree && dType == DitherType::No && !hasTransparentColor) {
        static_cast<OctreeQuantizer *>(colorQuantizer.get())->getColorIndices(quantizeIn, colorIndices);
    } else {
    ...
    if (hasTransparentColor) {
        GifBlockWriter::writeColorTableTransparency(content, transparentColorR, transparentColorG, transparentColorB);
